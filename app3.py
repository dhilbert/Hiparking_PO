...
class one_generation_statistics:
    ...
    def compare_one(self, dms_row, base_row):
        ...
        #  카드 + 취소(C) 일 때 → CreditCardApprovalNo 비교 제외
        if base_row.get("pay_method_type") == "CARD" and base_row.get("ApprovalType") == "C":
            compare += [
                ("CreditCardApprovalNo", "", base_row.get("CreditCardApprovalNo"))
            ]
        else:
            compare += [
